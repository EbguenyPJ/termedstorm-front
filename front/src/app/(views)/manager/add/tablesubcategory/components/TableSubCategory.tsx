...
const TableSubCategory: React.FC = () => {
  const [subCategories, setSubCategories] = useState<SubCategory[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [openModal, setOpenModal] = useState<string | undefined>();

  const subCategoryIdRef = useRef<string | null>(null);
  const [pendingDeleteSubCategoryName, setPendingDeleteSubCategoryName] = useState<string | null>(null);
  ...
  // Función para obtener las subcategorías con paginación
  const fetchSubCategories = async () => {
    ...
    try {
      ...
      if (response.data && typeof response.data === 'object' && 'total' in response.data) {
        setSubCategories(response.data.subcategories); // Asegúrate de que el key sea `subcategories`
        setTotalSubCategories(response.data.total);
      } else if (Array.isArray(response.data)) {
        setSubCategories(response.data);
        setTotalSubCategories(0);
        console.warn("La API devolvió un array directamente. No se puede determinar el total de subcategorías para una paginación completa. Por favor, considera actualizar tu API para que devuelva también el total de elementos (ej: { subcategories: [...], total: 100 }).");
      } else {
      ...
      setError('Error al obtener las subcategorías. Por favor, intenta de nuevo.');
      setSubCategories([]);
      setTotalSubCategories(0);
    } finally {
      setLoading(false);
  ...
  const handlePageChange = (page: number) => {
    ...
    if (page > 0 && (totalSubCategories === 0 || page <= Math.ceil(totalSubCategories / itemsPerPage))) {
      setCurrentPage(page);
    }
  };
  ...
  const handleEdit = (subCategory: SubCategory) => {
    ...
    setEditForm({ name: subCategory.name, image: subCategory.image });
    setIsEditing(true);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    const subCategoryId = currentSubCategory._id || (currentSubCategory as any).id;
    if (!subCategoryId) {
      console.error("Error: No se pudo obtener el ID de la subcategoría para actualizar.");
      return;
    }

    try {
      setLoading(true);
      await api.put(`/sub-categories/${subCategoryId}`, {
        name: editForm.name,
      ...
      setError('Error al actualizar la subcategoría. Por favor, intenta de nuevo.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteClick = (subCategory: SubCategory) => {
    const subCategoryId = subCategory._id || (subCategory as any).id;

    if (subCategoryId && typeof subCategoryId === 'string') {
      subCategoryIdRef.current = subCategoryId;
      setPendingDeleteSubCategoryName(subCategory.name);
      props.setOpenModal('pop-up');
    } else {
      console.error("Error: La subcategoría seleccionada no tiene un ID válido.", subCategory);
      setError('No se pudo seleccionar la subcategoría para eliminar. Por favor, intenta de nuevo.');
    }
  };

  const handleDeleteConfirmed = async () => {
    const idToDelete = subCategoryIdRef.current;

    if (!idToDelete) {
      console.error("Error interno: No se pudo obtener el ID de la subcategoría para eliminar.");
    ...
    try {
      ...
      setPendingDeleteSubCategoryName(null);
    } catch (err: any) {
      console.error("Error al eliminar la subcategoría:", err);
      setError('Error al eliminar la subcategoría. Por favor, intenta de nuevo.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const totalPages = totalSubCategories > 0 ? Math.ceil(totalSubCategories / itemsPerPage) : 0;

  if (isEditing && currentSubCategory) {
    return (
      <div className="div-container bg-base-100 p-8 rounded-lg shadow-lg">
        <h2 className="text-2xl font-bold text-base-300 mb-6">Editar Subcategoría</h2>
        <div className="space-y-4">
          <div>
            <Input
              ...
              placeholder="Introduce el nombre de la sub-categoría"
            />
          </div>
          <div className="mb-4">
        ...
        <div className="bg-base-100 relative shadow-md sm:rounded-lg overflow-hidden">
          <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
            ...
            <div className="w-full md:w-1/2">
              <h2 className="text-2xl font-bold text-base-300">Tabla de subcategorías</h2>
            </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              <div className="flex justify-center items-center p-8">
                ...
                <span className="ml-3 text-base-300">Cargando subcategorías...</span>
              </div>
            ) : error ? (
              <div className="text-center p-8 text-accent">
                <p>Error: {error}</p>
                <button onClick={fetchSubCategories} className="mt-4 text-primary underline">
                  Intentar de nuevo
                </button>
              </div>
              ...
              <table className="w-full text-sm text-left text-base-250">
                ...
                <tbody>
                  {subCategories.map((subCategory) => (
                    ...
                    <tr key={subCategory._id || (subCategory as any).id} className="border-b border-base-200 hover:bg-base-200">
                      ...
                      <td className="px-4 py-3 flex items-center justify-end">
                        ...
                        >
                          <DropdownItem onClick={() => handleEdit(subCategory)} className="hover:bg-base-200 text-base-300">
                            Editar
                          </DropdownItem>
                          <DropdownItem onClick={() => handleDeleteClick(subCategory)} className="hover:bg-accent hover:text-base-100 text-base-300">
                            Eliminar
                          </DropdownItem>
                        </Dropdown>
                      </td>
          ...
          <nav className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
            <span className="text-sm font-normal text-base-250">
              {/* Muestra el rango de elementos si totalSubCategories es conocido */}
              {totalSubCategories > 0 ? (
                <>
                  ...
                  <span className="font-semibold text-base-300">
                    ...
                    -
                    {currentPage * itemsPerPage > totalSubCategories ? totalSubCategories : currentPage * itemsPerPage}
                  </span>
                  de
                  <span className="font-semibold text-base-300">
                    {" " + totalSubCategories}
                ...
                `Mostrando ${subCategories.length} subcategorías en esta página.`
              )}
            </span>
            <ul className="inline-flex items-stretch -space-x-px">
              ...
              <li>
                <a
                  href="#"
                  className={`flex items-center justify-center h-full py-1.5 px-3 leading-tight text-base-250 rounded-r-lg border border-base-200 hover:bg-base-200 hover:text-base-300 ${currentPage === totalPages || totalSubCategories === 0 && subCategories.length < itemsPerPage ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                  onClick={(e) => { e.preventDefault(); handlePageChange(currentPage + 1); }}
                >
