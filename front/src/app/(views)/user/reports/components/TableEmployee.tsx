...
const TableEmployee = () => {
  ...
  const [rolesDisponibles, setRolesDisponibles] = useState<IRole[]>([]);
 ...
 const fetchEmployees = async () => {
  ...
  return (
    <div className="mt-10">
      <h2 className="text-xl font-semibold mb-4 text-primary">Empleados</h2>
            ...
            <tr key={emp.userId} className="border-t border-gray-300 hover:bg-gray-50">
              ...
              <td className="p-3 text-center">
                <button
                  onClick={() => {
                    setEditing(emp);
                    const matchedRoles = rolesDisponibles.filter((r) =>
                      emp.roles.includes(r.name)
                    );
