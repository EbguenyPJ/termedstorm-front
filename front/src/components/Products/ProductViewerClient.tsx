        ...
        {variants.map((v, i) =>
          v.image.map((img, idx) => (
            <div
              ...
              className={`w-20 h-20 relative rounded overflow-hidden border cursor-pointer ${
                img === selectedImage ? "border-primary" : "border-gray-300"
              }`}
            >
              <Image
                src={img}
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            {Array.from(new Set(variants.map((v) => v.color_id))).map(
              (colorId, index) => (
                <button
                  key={index}
                  onClick={() => {
            ...
            {currentVariant?.variantSizes?.map((vs, index) => (
              <label
                ...
                className={`px-3 py-1 rounded border cursor-pointer ${
                  selectedSizes.includes(vs.size_id)
